...
set.seed(1)
# idx <- sample(1:nrow(mydata), nrow(mydata) * 0.8)
idx <- sample(1:nrow(mydata), nrow(mydata) * 0.7)
# idx <- sample(1:nrow(mydata), nrow(mydata) * 0.6)
train <- mydata[idx, ]
test <- mydata[-idx, ]
dim(train)  # 280   4
dim(test)  # 120   4
...
t <- table(result_pred, test$admit)  # 예측값, 실제값
t
(77 + 11) / nrow(test)
(t[1,1] + t[2,2])/ nrow(test)  # 0.7333333   73.3%의 분류정확도
